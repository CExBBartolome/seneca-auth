...
var sendredirect      = _sendredirect.bind(null, null)
...
function ServiceRouter(options, seneca, plugin, passport) {
  ...
  var hasAllowedHosts = options && options.redirect && options.redirect.allowedHosts &&
	  _.isArray(options.redirect.allowedHosts);
  if(hasAllowedHosts) {
	  sendredirect = _sendredirect.bind(null, _.pick(options.redirect, ['fail', 'allowedHosts']))
  }
...
ServiceRouter.prototype._authRequest = function(req, res, next) {
  ...
  if( prefix ) {
    res.seneca.cookies.set(self._options.transientPrefix + 'url-prefix', prefix, self._options.cookies)
  }

  if( context ) {
    res.seneca.cookies.set(self._options.transientPrefix + 'context', context, self._options.cookies)
  }
...
ServiceRouter.prototype._redirection = function(req, res, kind) {
  ...
  var cookieRedirectWin = req.seneca.cookies.get(this._options.transientPrefix + 'redirect-win')

  if( this._options.redirect.always ) {
    redirect = true
  } else if( !_.isUndefined(req.query.redirect) ) {
    redirect = true
  } else if( !_.isUndefined(cookieRedirectWin) ) {
    redirect = true
  ...
  if( redirect ) {
    ...
    var win = transientPrefix + this._options.redirect.win
    if(_.isString(req.query.win)) {
      win = req.query.win
    } else if(cookieRedirectWin) {
      req.seneca.cookies.set(this._options.transientPrefix + 'redirect-win')
      win = cookieRedirectWin
    } else if(rk && rk.win) {
      win = transientPrefix + rk.win
    }

    var fail = transientPrefix + this._options.redirect.fail
    if(_.isString(req.query.fail)) {
      fail = req.query.fail
    } else if(rk && rk.fail) {
      fail = transientPrefix + rk.fail
    }
...
//    this._adminLocal(req, req.seneca.user)
    ...
    res.seneca.cookies.set(this._options.tokenkey, req.seneca.login.id, this._options.cookies)
...
function _sendredirect(opts, code, res, url) {
  if(opts) {
    var host = urlParser(url)['host']
	 if(host && !_.contains(opts.allowedHosts, host)) {
		 url = opts.fail
	 }
  }
